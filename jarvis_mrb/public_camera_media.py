...
def validate_public_camera_url(url: str, *, provider_signed: bool = False) -> MediaTarget:
    ...
    if port not in (None, 443):
        raise ValueError("Camera URL must use HTTPS port 443.")
    ...
    display = "https://" + hostname + (parts.path or "/")
...
def _get(target: MediaTarget, *, limit: int, accept: str,
         provider_signed: bool = False) -> tuple[bytes, str]:
    # Validate again for each playlist/segment GET, including relative URLs.
    target = validate_public_camera_url(target.url, provider_signed=provider_signed)
    ip = _public_address(target.host)
    con = _PinnedTLS(target, ip)
    try:
...
def _hls_media(url: MediaTarget, manifest: bytes, *, signed: bool,
    ...
    if any(x.startswith("#EXT-X-STREAM-INF") for x in lines):
        ...
        child, mime = _get(item, limit=_MAX_PLAYLIST,
                           accept="application/vnd.apple.mpegurl", provider_signed=signed)
...
def _same_origin(base: MediaTarget, relative: str, *, signed: bool) -> MediaTarget:
...
def snapshot_public_media(url: str, *, provider_signed: bool = False) -> dict[str, Any]:
    """Inspect one explicitly selected public frame; never store raw imagery."""
    target = validate_public_camera_url(url, provider_signed=provider_signed)
    ...
    if mime in _HLS_TYPES or target.url.split("?", 1)[0].endswith(".m3u8"):
        raw = _hls_media(target, raw, signed=provider_signed)
        mime = "image/jpeg"
