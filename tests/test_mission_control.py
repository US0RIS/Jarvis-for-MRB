...
class MissionControlContractTests(unittest.TestCase):
    ...
    def test_source_compiles_in_phone_with_first_class_mission_board(self):
        ...
        self.assertIn("Cross-system goal missions", self.mission)
        self.assertIn("guardian.refreshObjectiveWatches()", self.mission)
        self.assertIn("guardian.stageObjectiveFollowup(watch.id)", self.mission)
        self.assertIn("guardian.revokeObjective(watch.id)", self.mission)
