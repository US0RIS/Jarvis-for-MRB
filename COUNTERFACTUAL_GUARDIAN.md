...
The Guardian shows a source- and time-labelled warning for supported impending departure risks, deduplicates events within the current session, and avoids speaking when driving, in meetings, or in active conversations. A HUD card is redacted by default. Guardian does not send raw phone GPS/route history to the Jarvis Windows backend. **Apple's MapKit service necessarily receives the route origin and destination to resolve a place and calculate a driving ETA** and is subject to Apple's own processing/privacy terms. The iPhone holds the matched calendar information and resolved route in memory, rather than mirroring them to the general Jarvis world model.
