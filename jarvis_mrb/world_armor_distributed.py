...
"""Explicit distributed World Armor movement scheduler over trusted Mesh nodes.

The Windows controller remains the sole authority for source grants, provider
cadence, leases, retention and evidence writes. Paired Macs may perform only
the fixed read-only source task their startup capability advertises.

No network discovery, arbitrary URL, arbitrary RPC, remote shell, credential
delegation, camera recording, person tracking or action authority is added.
"""
...
def available_workers() -> dict[str, Any]:
    ...
    for node in fabric.get("nodes") or []:
        ...
        if (node_id in ("macbook", "macmini")
                and node.get("status") == "online"
                and isinstance(capabilities, dict)
                and capabilities.get("world_observer")
...
def run_due(*, db_path: Path | None = None, limit: int = 8,
    ...
    """One bounded controller dispatch pass; source count itself is not capped."""
    _require_enabled()
