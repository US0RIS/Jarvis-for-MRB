...
After enabling **Guard calendar departures proactively** (off by default) plus **Motion / travel context and GPS**, the foreground iPhone fetches bounded, authenticated *primary* Google Calendar event IDs/titles/start times/literal locations from the Windows backend at most once per two minutes. Only the phone coordinates the unique place-resolution and ETA request using a fresh accurate Core Location fix; Apple MapKit receives the origin/destination to provide its routing service, but the Jarvis Windows backend does not receive the GPS fix or route trail. Missing/ambiguous/stale evidence is never called an all-clear. The deterministic five-minute departure buffer is explicitly stated.
