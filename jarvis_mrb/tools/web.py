...
def web_answer(query: str, *, num: int = 5) -> WebResult:
    """Search the web, escalating selection-risk requests to audited research."""
    original = " ".join(query.strip().split())
    if not original:
        return WebResult(False, "Web search requires a query.")

    # The receipt is generated by the execution layer, not by the conversational
    # model. A user can therefore ask to inspect the actual search trace later.
    receipt_request = original.lower()
    if (
        "research receipt" in receipt_request
        or "search receipt" in receipt_request
        or "show me how you searched" in receipt_request
        or "prove you searched" in receipt_request
    ):
        from jarvis_mrb.search_integrity import research_receipt
        detail = "full" if any(cue in receipt_request for cue in ("full", "complete", "json")) else "summary"
        message = research_receipt("latest", detail=detail)
        return WebResult(True, message, {"kind": "research_receipt"})

    from jarvis_mrb.search_integrity import audited_research, requires_audited_research
    if requires_audited_research(original):
        audited = audited_research(original, num_per_query=10, synthesize=True)
        return WebResult(audited.ok, audited.message, audited.data)
