...
- `/mesh/graph/fabric`: compile live status of the explicitly paired Windows/Mac nodes alongside the *nonlive* public-source registry. Actual node health timestamps, capability flags and unknown/offline status are modeled without turning descriptive capability into execution authority or treating registered cameras as verified streams.
- The iPhone/iPad **Mesh → selected place → Compile Reality Graph** panel shows model-free place facts, provider gaps, evidence freshness and evaluation time after a separate authenticated source check. It resets on place change or Privacy mode.
