...
**Next major development target: [Jarvis Conductor](NEXT_BIG_UPGRADE.md).** Conductor is selected, **not yet deployed**: it will bind live Reality Mesh node capabilities to mission-specific, limited-use approvals and independently verify the actual intended state across multiple hosts. The first release gate is a real, exact-device workstation preparation mission from iPad to Windows/Mac, with source-timestamped observations after each authorized action. The complete direction and exclusions are in the linked spec.
