...
- **A6:** Start while the desired state is blocked and has at least one persisted active wake watch. Change the blocking condition through direct real observed evidence for the watched criterion; an unrelated external event on the same entity does not count. After reactivation, Agency must automatically execute or surface the newly feasible next step according to permissions.
