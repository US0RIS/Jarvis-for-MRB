...
def _prepare_email(arguments: dict[str, Any]) -> AgentReply:
    ...
    return AgentReply(
        True,
        f"Ready to send email to {name} <{email}>.{subject_line} Body: {body!r} Type 'confirm' to send or 'cancel'.",
    )
...
def _confirm_pending() -> AgentReply:
    ...
    result = send_email(
        ...
        body=pending["body"],
        subject=pending.get("subject") or None,
    )
...
def _fast_path(text: str) -> AgentReply | None:
    ...
    status_match = re.fullmatch(
        r"(?:is|check if|check whether) (.+?) (?:running|open)\??",
        normalized,
    )
    if status_match:
        return _execute_tool("pc.app_status", {"name": status_match.group(1)})

    close_match = re.fullmatch(r"(?:close|quit|stop|kill) (.+?)[?.!]?", normalized)
    if close_match:
        return _execute_tool("pc.close_app", {"name": close_match.group(1)})
    ...
    if open_match:
        ...
        return _execute_tool("pc.launch_app", {"name": target})

    # Keep the tested Minecraft phrasing instant.
    if normalized in {
        "minecraft status",
        "check minecraft",
...
Tools:
- pc.app_status {name}: check whether an app/process is running
- pc.launch_app {name}: launch an application
- pc.close_app {name}: close an application
...
User: Get Spotify open.
Assistant: {"tool":"pc.launch_app","arguments":{"name":"Spotify"},"response":""}
User: Close Discord.
Assistant: {"tool":"pc.close_app","arguments":{"name":"Discord"},"response":""}
