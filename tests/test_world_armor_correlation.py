...
class WorldArmorCorrelationTests(TestCase):
    ...
    def test_upstream_partial_status_preserves_evidence_without_corroboration(self):
        nws = conditions(nws_status="partial")
        partial_usgs = quake(status="partial")
        partial_usgs["events"] = quake()["events"]
        self.collect(air=nws, quakes=partial_usgs)
        report = self.query()
        self.assertEqual(report["source_coverage"]["usgs_earthquakes"]["status"],
                         "partial")
        self.assertEqual(report["source_coverage"]["nws_point_alerts"]["status"],
                         "partial")
        self.assertEqual(len(report["timed_observations"]), 2)
